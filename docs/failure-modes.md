...
1. **Jump-table entry width** → `value evaluated as <N> is out of range`.
   ...
   *given correct instruction sizes*. The workarounds (`+force-32bit-jump-tables`
   and per-TU `/Ob1`) have been REMOVED from `Build-OpencvFromSource.ps1`. The
...
`Build-LlvmFromSource.ps1` asserts on the PATCHED CONTENT afterwards
...
**Fix.** `-DLLVM_ENABLE_DIA_SDK=OFF` (already in `Build-LlvmFromSource.ps1`). DIA
