...
3. **Embeddings — the biggest prize, entirely unbuilt.** Embedding is pure
   ...
   [`qnn-linux.md`](qnn-linux.md), because `/dev/fastrpc*` does not exist
   inside WSL2.
...
| Zero-model control | tesseract `deu+eng` via ocrmypdf | – | Apache-2.0 | WSL2 | Recipe at [`linux-reference.md`](linux-reference.md) § OCR. Deterministic; produces detectable garbage rather than plausible fiction. **If no VLM beats it on your own pages, ship it.** |
...
- **A second machine may exist.** [`build-parallelism-memory-tuning.md`](build-parallelism-memory-tuning.md)
