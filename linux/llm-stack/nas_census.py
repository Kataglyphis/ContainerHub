...
docs/nas-document-ai.md § 6 day 1 (and § 8's recorded dissent) is why this
tool exists: every sizing decision on that page is a linear function of the
scanned fraction, published corpora span 5-44 % scanned, a German household
NAS may be 60-70 % — and nobody has the number. So this walks a tree and
publishes THE FOUR NUMBERS — total PDF pages, scanned fraction, German
fraction, table density — plus the gate the doc prescribes: if scanned +
image-only is under ~10 % of classified pages, the VLM is a footnote and the
budget belongs to extraction + embeddings + retrieval.
...
# ---------------------------------------------------------------------------
# Gates, from docs/nas-document-ai.md § 6. Constants so the mutation gate can
# bite them and so a future recalibration is one diff line, not a spelunk.
# ---------------------------------------------------------------------------
...
GERMAN_STOPWORDS = frozenset(
    "der die das und ist nicht mit für auf ein eine den dem von zu im sich".split())
ENGLISH_STOPWORDS = frozenset(
    "the and is of to in that for with on as this was are be it".split())
...
def _zip_xml_texts(path, members):
    ...
    with zipfile.ZipFile(path) as zf:
        for member in members:
            parts.extend(_local_texts(ET.fromstring(zf.read(member))))
    return " ".join(parts)
    ...
    try:
        ...
        with zipfile.ZipFile(path) as zf:
            ...
            if "xl/sharedStrings.xml" in names:
                parts.extend(_local_texts(ET.fromstring(zf.read("xl/sharedStrings.xml"))))
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Census a document tree: THE FOUR NUMBERS and the VLM gate "
                    "of docs/nas-document-ai.md § 6, before any model is chosen.")
