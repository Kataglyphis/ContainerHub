...
_subjects() {
  ...
  printf 'import sys\n' > "${d}/linux/scripts/verify_lambda.py"
  ...
  printf 'x verify_lambda.py\n' > "${d}/linux/scripts/tests/test-lambda.sh"
...
EOF
  cat > "${d}/linux/host-config/git-hooks/pre-push" <<'EOF'
python3 linux/scripts/verify_lambda.py --stale-check
EOF
  cat > "${d}/linux/scripts/preflight.sh" <<'EOF'
KNOWN_SLUGS=(alpha beta gamma omega delta \
             epsilon zeta eta theta iota kappa lambda script-tests)
run_check() { :; }
...
run_check lambda "lambda gate" ${PREFLIGHT_PYTHON} linux/scripts/verify_lambda.py
...
t_assert_contains "$(_run "${fix}")" "13 slugs; 11 proven; 2 unproven, 2 frozen" "the census line"
...
t_assert_contains "$(_row "${fix}" eta)" "| hook (whole tree)+CI |" \
  "an inline gate the hook calls by function name, at top level: nothing scopes it to the commit"
...
t_assert_eq "0" "$(_count_in_row "${fix}" lambda '| CI |')" \
  "a second hook that gates every push is not 'CI-only'"
...
fix="$(_fixture "$(printf 'delta\nomega')")"
sed -i 's/ theta iota kappa lambda script-tests)/ iota kappa lambda script-tests)/' "${fix}/linux/scripts/preflight.sh"
