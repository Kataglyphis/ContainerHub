...
RUN_CONFIG="${ENV_CONFIG}" _run "${ENV_REPO}" "${RUST_REPORT}" \
  --apply --managers custom.regex
...
RUN_CONFIG="${ENV_CONFIG}" _run "${ENV_REPO}" "${CMAKE_REPORT}" \
  --apply --managers custom.regex
...
RUN_CONFIG="${ENV_CONFIG}" _run "${ENV_REPO}" "${NODE_REPORT}" \
  --apply --managers custom.regex
...
printf '%s\n' '# renovate: datasource=github-tags depName=rust-lang/rust' \
  > "${BARE_REPO}/${ENV_REL}"
_commit "${BARE_REPO}"
RUN_CONFIG="${ENV_CONFIG}" _run "${BARE_REPO}" "${RUST_REPORT}" \
  --apply --managers custom.regex
