#!/usr/bin/env bash
# TEMPLATE — copy to <consumer>/scripts/linux/lib/antfrastructure.sh.
#
# Do NOT source this file out of the submodule: like its PowerShell twin
# shared/windows/templates/Resolve-BuildModule.ps1, this is the one piece of
# build tooling that cannot be consumed from ANTfrastructure, because it is what
# *finds* ANTfrastructure.
#
# The template exists for the same reason the PowerShell one does. On 2026-08-11
# six consumers were found to have six different bootstraps — source_module() in
# a lib/common.sh, antfrastructure_path/antfrastructure_source, a bare
# _ANTFRASTRUCTURE_CORE variable, a _DRIVER variable inlined into every wrapper,
# ANTFRASTRUCTURE_SETUP_SCRIPT, and ANTFRASTRUCTURE_DIR/ANTFRASTRUCTURE_SCRIPTS_DIR — with
# different search orders, different error text, and different working-directory
# assumptions. One of them sourced a path that had moved upstream and failed with
# nothing but bash's "No such file or directory".
#
# Bash has no bootstrap problem the way PowerShell does — it can source by
# relative path — but it has a working-directory problem: a relative path only
# resolves when the caller happens to sit in the repo root. Everything here
# resolves from ${BASH_SOURCE[0]}, so it works from any cwd, and fails loudly
# naming the probed path and the fix.
#
# Contract:
#
#   antfrastructure_path <relative>   -> absolute path, or fail naming it
#   antfrastructure_source <relative> -> source a library (load-guarded upstream)
#   antfrastructure_exec <relative>   -> exec a driver, passing "$@" through
#
# All three take paths relative to the ANTfrastructure root, e.g.
# "linux/scripts/01-core/logging.sh".
#
# ADJUST: KATAGLYPHIS_REPO_ROOT_RELATIVE below if this file does not sit exactly
# three directories below the repo root (scripts/linux/lib/).
