...
# upstream is importable.
#
# The template exists because all four consumers had independently written this
# file and they had drifted — different search orders, different error text,
# one of them silently missing the -Global flag. Copying one file is fine;
# copying four different ones is the failure mode.
#
# Contract (docs/adopting-in-a-new-project.md § 1):
#
...
# automatically, so a consumer never silently keeps building against a stale
# vendored copy. Keep ONLY genuinely project-specific modules in the local
# fallback directory.
#
# ADJUST: $script:RepoRootRelativeToHere if this script does not sit exactly two
